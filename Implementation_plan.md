...
- [ ] Add the full Herdr plugin system. OSC 8 hyperlink metadata is now tracked
  ...
  `pane.closed`, `pane.exited`, `pane.moved`, `layout.updated`, `tab.created`, `tab.focused`, `tab.closed`, `workspace.focused`, `workspace.created`, `workspace.renamed`, and `workspace.closed`, delivered from the server event history with the
