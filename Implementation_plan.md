...
- [ ] Add the full Herdr plugin system. OSC 8 hyperlink metadata is now tracked
  ...
  `pane.closed`, `pane.exited`, `tab.created`, `tab.focused`, `tab.closed`, `workspace.focused`, `workspace.created`, and `workspace.renamed`, delivered from the server event history with the
