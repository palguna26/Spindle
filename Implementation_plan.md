...
- [ ] Port process/terminal agent detection using Herdr's manifests and state
  ...
  Recent Amp, Antigravity, Cline, Cursor, Devin, Droid, Grok, Hermes, Kilo, Kiro, Kimi, Maki, Qoder CLI, and Qwen rule functions are now grouped under
  `src/detect/agents/`, matching Herdr's per-agent manifest organization.
  direct and wrapped Pi/Qoder CLI/Droid/Grok/Hermes/Kilo/Kiro/Kimi/Maki/Qwen/Cline/Devin/Cursor/Amp/Antigravity/Claude/Codex/Gemini/
  ...
  blocker cues, Qwen prioritized title/screen/OSC state rules, Grok permission and title/OSC/footer cues, Maki permission/status-bar/prompt cues, and Copilot selection blocker,
