...
- [ ] Port process/terminal agent detection using Herdr's manifests and state
  ...
  Recent Cline, Cursor, Devin, Droid, Kiro, Kimi, and Qoder CLI rule functions are now grouped under
  `src/detect/agents/`, matching Herdr's per-agent manifest organization.
  direct and wrapped Pi/Qoder CLI/Droid/Kiro/Kimi/Cline/Devin/Cursor/Claude/Codex/Gemini/
  ...
  Windows Node install-path recognition and status cues, and Copilot selection blocker,
