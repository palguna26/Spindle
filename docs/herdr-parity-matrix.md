...
| Select/copy terminal text and route terminal mouse input | Partial | `src/selection.rs`, `src/client/shell/mouse.rs` | Spindle supports visible-screen drag selection, highlighting, and native clipboard copy (OSC 52 fallback). It tracks app-requested mouse modes and forwards supported SGR/legacy events. Scrollback selection, word selection, and live Windows Terminal verification remain. |
