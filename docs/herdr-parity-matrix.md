...
| Right-click context menus for pane/tab/workspace actions | Partial | `src/client/shell/context_menu.rs` | Spindle opens target-specific menus with keyboard and mouse selection, bounded placement, and tested small-terminal scrolling. Pane split, close, rename, focus, and toggle-zoom actions are implemented. Herdr-only worktree actions and right-click passthrough remain missing. |
