...
| Click panes/tabs/workspaces; drag split borders | Partial | `src/client/shell/mouse.rs`, `src/layout.rs` | Spindle handles clicks and divider drags. Drag updates the exact nested split by tree path, saves the ratio, and triggers per-pane PTY resizing. Unit and server-client tests cover hit paths and persisted nested ratios; terminal-app mouse passthrough and manual Windows Terminal verification remain. |
