...
Agent hook parity now includes `spindle pane report-agent`,
`spindle pane clear-agent-authority`, and `spindle pane release-agent`. Reports
are source/sequence aware, and reported
...
Codex/OpenCode/Gemini/Cline/Copilot/Pi/Qoder CLI/Droid/Devin/Cursor/Amp/Antigravity/Kilo/Hermes/Kiro/Kimi/Maki/Muse/Grok/Qwen/Claude detection parity note: `src/detect/manifest.rs` now
...
bars, background-agent cues, and stale-scrollback tests. Pi’s exact
`Working...` rule is also covered, along with Qoder’s permission, cancel, and spinner rules,
and Droid’s selection-menu, stop-hint, and spinner rules, plus Devin’s trust,
permission, working-footer, and idle-prompt rules, plus Cursor’s file-approval,
command-approval, background-task, stop-hint, and spinner rules, plus Amp’s
OSC-title, approval, footer, cancel, and idle rules, and Antigravity’s
permission, spinner, and background-task rules, and Kilo’s permission-menu and
interrupt rules, and Hermes’ title-priority, approval, clarification,
...
narrow-pane prompt rules, and Muse’s trust, picker, menu-overlay, approval,
working, and idle fallback rules, and Grok’s title, progress, dialog, background,
working, and idle rules. Qwen’s title, locale confirmation, dialog, cancel,
