...
Codex/OpenCode detection parity note: `src/detect/manifest.rs` now evaluates
Herdr-style manifest slices with priority ordering, OSC title rules, top/bottom
screen regions, directory trust validation, prompt-scoped blockers, permission
controls, interrupt hints, progress bars, and stale-scrollback tests. Other
agent detectors remain on compatibility functions until migrated to the shared
evaluator.
