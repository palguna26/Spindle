...
| Right-click context menus for pane/tab/workspace actions | Partial | `src/client/shell/context_menu.rs` | Spindle opens target-specific menus with keyboard and mouse selection, bounded placement, and tested small-terminal scrolling. Live PowerShell verified pane right-click and Split right. Herdr-only worktree actions, pane zoom, and right-click passthrough are not implemented. |
