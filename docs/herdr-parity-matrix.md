...
| Start with a live shell; create shell-backed tabs/workspaces | Partial | `src/workspace.rs` (`Workspace::new`, `create_tab_with_runtime`) | Spindle ensures a pane at UI attach (`src/client/app.rs`) but server model creates empty tabs/workspaces; add creation and reattach integration tests. |
