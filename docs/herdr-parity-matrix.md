...
| Render pane layouts with focus, terminal state, correct geometry | Partial | `src/ui/panes.rs`, `src/layout.rs` | Spindle stores and restores focused pane per tab, matching Herdr's focus in each tab layout. Visible sidebar/click focus is still missing; `resize_panes` also resizes every PTY to whole-terminal size. Verify rectangles and per-pane PTY dimensions. |
