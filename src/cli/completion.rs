...
const BASH: &str = r#"_spindle() {
  ...
  if (( COMP_CWORD == 1 )); then
    ...
    if [[ "$COMP_WORDS[2]" == "install" || "$COMP_WORDS[2]" == "uninstall" ]]; then
      COMPREPLY=( $(compgen -W "codex opencode claude" -- "$cur") )
    else
...
complete -c spindle -f -n '__fish_seen_subcommand_from integration; and __fish_seen_subcommand_from install uninstall' -a 'codex opencode claude'
...
_spindle() {
  ...
  case $words[2] in
    ...
    integration) _arguments '1:command:(status install uninstall help)' '2:target:(codex opencode claude)' '3:options:(--json)' ;;
  esac
}
_spindle "$@"
"#;
...
const POWERSHELL: &str = r#"Register-ArgumentCompleter -Native -CommandName spindle -ScriptBlock {
  ...
  $choices = if ($words.Count -le 1) { 'start attach stop list status doctor config workspace worktree tab pane agent notification integration completion api help' }
    ...
    elseif ($words[1] -eq 'integration' -and ($words[2] -eq 'install' -or $words[2] -eq 'uninstall')) { 'codex opencode claude' }
