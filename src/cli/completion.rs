...
const BASH: &str = r#"_spindle() {
  ...
  if (( COMP_CWORD == 1 )); then
    ...
    COMPREPLY=( $(compgen -W "list get focus wait read send-keys help" -- "$cur") )
...
complete -c spindle -f -n '__fish_seen_subcommand_from agent' -a 'list get focus wait read send-keys help'
"#;
...
_spindle() {
  ...
  case $words[2] in
    ...
    agent) _arguments '1:command:(list get focus wait read send-keys help)' ;;
  esac
}
_spindle "$@"
"#;
...
const POWERSHELL: &str = r#"Register-ArgumentCompleter -Native -CommandName spindle -ScriptBlock {
  ...
  $choices = if ($words.Count -le 1) { 'start attach stop list doctor config workspace tab pane agent completion api help' }
    ...
    elseif ($words[1] -eq 'agent') { 'list get focus wait read send-keys help' }
...
edit:completion:argadd 'spindle agent' (list get focus wait read send-keys help)
"#;
