...
pub fn agent_sidebar_scroll_region(
    area: Rect,
    collapsed: bool,
    split_ratio: f32,
    x: u16,
...
pub fn agent_sidebar_scroll_thumb_grab_offset(
    ...
    if !agent_sidebar_scroll_region(area, collapsed, split_ratio, x, y) {
        return None;
    }
    let rows = sidebar_rows_with_collapsed(snapshot, agent_priority_sort, collapsed_groups);
    let agents = rows
...
fn render_tab_bar_right(
    ...
    let mut spans = Vec::with_capacity(parts.len().saturating_mul(2));
    for (index, (text, accent)) in parts.iter().enumerate() {
        ...
        let style = if *accent {
            ...
            Style::default().fg(super::ThemePalette::overlay1(config))
        };
