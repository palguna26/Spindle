...
pub fn hit_test_navigator(
    ...
    if navigator.is_mobile() {
        return hit_test_mobile_navigator(snapshot, navigator, terminal, x, y);
    }
...
fn render_mobile_navigator(
    ...
    lines.push(Line::from(vec![
        ...
        Span::raw(" ".repeat(usize::from(area.width.saturating_sub(15)))),
    ...
    lines.push(Line::from(vec![
        ...
        Span::raw(if navigator.search_focused() {
            ...
            "Search spaces, workspaces, tabs, panes".to_owned()
        } else {
            navigator.query().to_owned()
        }),
    ]));
    ...
    lines.push(Line::from(Span::styled(
        " j/k move · Space expand · / search · Enter open · Esc close ",
        Style::default().fg(Color::DarkGray),
    )));
    frame.render_widget(Clear, area);
...
fn hit_test_mobile_navigator(
    ...
    if x < terminal.x || x >= terminal.right() || y < terminal.y || y >= terminal.bottom() {
        return Hit::Outside;
    }
    if y == terminal.y && x >= terminal.right().saturating_sub(10) {
        return Hit::Close;
    }
    if y == terminal.y.saturating_add(1) {
        return Hit::Search;
    }
    let visible = usize::from(terminal.height.saturating_sub(3));
    let rows = navigator.rows(snapshot);
    let selected_index = navigator
        .selected(snapshot)
        .as_ref()
    ...
    let row_index = usize::from(y.saturating_sub(terminal.y.saturating_add(2)));
    if row_index >= visible {
        return Hit::Inside;
    }
    rows.get(start + row_index)
...
mod tests {
    ...
    #[test]
    fn mobile_navigator_has_search_and_close_hit_regions() {
        let snapshot = Session::default().snapshot().clone();
        let navigator = Navigator::new_mobile(&snapshot);
        let terminal = Rect::new(0, 0, 44, 20);
        assert_eq!(
            hit_test_navigator(&snapshot, &navigator, terminal, 40, 0),
            Hit::Close
        );
        assert_eq!(
            hit_test_navigator(&snapshot, &navigator, terminal, 4, 1),
            Hit::Search
        );
        assert!(matches!(
            hit_test_navigator(&snapshot, &navigator, terminal, 4, 2),
            Hit::Row { .. }
        ));
    }
}
