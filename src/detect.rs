...
pub(crate) fn detect_state_with_osc(
    ...
    if agent == AgentKind::GithubCopilot {
        if let Some(state) = manifest::detect_copilot(manifest::DetectionInput {
            screen,
            osc_title: title,
            _osc_progress: osc_progress,
        }) {
    ...
    if agent == AgentKind::Pi {
        if let Some(state) = manifest::detect_pi(manifest::DetectionInput {
            screen,
            osc_title: title,
            _osc_progress: osc_progress,
        }) {
    ...
    if agent == AgentKind::QoderCli {
        if let Some(state) = manifest::detect_qoder(manifest::DetectionInput {
            screen,
            osc_title: title,
            _osc_progress: osc_progress,
        }) {
    ...
    if agent == AgentKind::Droid {
        if let Some(state) = manifest::detect_droid(manifest::DetectionInput {
            screen,
            osc_title: title,
            _osc_progress: osc_progress,
        }) {
    ...
    if agent == AgentKind::Devin {
        if let Some(state) = manifest::detect_devin(manifest::DetectionInput {
            screen,
            osc_title: title,
            _osc_progress: osc_progress,
        }) {
    ...
    if agent == AgentKind::Cursor {
        if let Some(state) = manifest::detect_cursor(manifest::DetectionInput {
            screen,
            osc_title: title,
            _osc_progress: osc_progress,
        }) {
    ...
    if agent == AgentKind::Antigravity {
        if let Some(state) = manifest::detect_antigravity(manifest::DetectionInput {
            screen,
            osc_title: title,
            _osc_progress: osc_progress,
        }) {
    ...
    if agent == AgentKind::Kilo {
        if let Some(state) = manifest::detect_kilo(manifest::DetectionInput {
            screen,
            osc_title: title,
            _osc_progress: osc_progress,
        }) {
    ...
    if agent == AgentKind::Hermes {
        if let Some(state) = manifest::detect_hermes(manifest::DetectionInput {
            screen,
            osc_title: title,
            _osc_progress: osc_progress,
        }) {
    ...
    if agent == AgentKind::Kiro {
        if let Some(state) = manifest::detect_kiro(manifest::DetectionInput {
            screen,
            osc_title: title,
            _osc_progress: osc_progress,
        }) {
    ...
    if agent == AgentKind::Kimi {
        if let Some(state) = manifest::detect_kimi(manifest::DetectionInput {
            screen,
            osc_title: title,
            _osc_progress: osc_progress,
        }) {
