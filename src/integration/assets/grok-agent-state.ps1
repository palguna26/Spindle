...
$event = if ($null -ne $payload -and $payload.hook_event_name -is [string]) { $payload.hook_event_name } elseif ($null -ne $payload -and $payload.hookEventName -is [string]) { $payload.hookEventName } else { $null }
if ($null -ne $event -and $event -notin @("session_start", "SessionStart", "sessionStart")) { exit 0 }
$sessionId = $env:GROK_SESSION_ID
if ([string]::IsNullOrWhiteSpace($sessionId) -and $null -ne $payload -and $payload.session_id -is [string]) { $sessionId = $payload.session_id }
...
try { & $bin pane report-agent-session $paneId --source spindle:grok --agent grok --seq ([DateTimeOffset]::UtcNow.ToUnixTimeMilliseconds()) --agent-session-id $sessionId 2>$null | Out-Null } catch {}
