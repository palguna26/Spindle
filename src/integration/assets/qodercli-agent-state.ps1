...
# SPINDLE_INTEGRATION_ID=qodercli
...
try { & $bin pane report-agent-session $paneId --source spindle:qodercli --agent qodercli --agent-session-id $payload.session_id --seq ([DateTimeOffset]::UtcNow.ToUnixTimeMilliseconds()) 2>$null | Out-Null } catch {}
