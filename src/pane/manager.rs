...
impl PaneManager {
    ...
    pub(crate) fn report_display_agent(
        &mut self,
        id: &str,
        source: String,
        ...
        let pane = self
            ...
            .ok_or_else(|| PaneManagerError::MissingPane(id.into()))?;
        if pane
            .display_agent_authority
            .as_ref()
            .is_some_and(|authority| !authority.accepts(&source, seq))
        {
            return Ok(false);
        }
        if clear == display_agent.is_some() {
            return Ok(false);
        }
        let next = display_agent.filter(|value| !value.trim().is_empty());
        let changed = pane.display_agent != next;
        pane.display_agent = next;
        pane.display_agent_authority = Some(AgentAuthority { source, seq });
        Ok(changed)
    }
    ...
    pub(crate) fn report_display_state_labels(
        &mut self,
        id: &str,
        source: String,
        ...
        let pane = self
            ...
            .ok_or_else(|| PaneManagerError::MissingPane(id.into()))?;
        if pane
            .display_state_labels_authority
            .as_ref()
            .is_some_and(|authority| !authority.accepts(&source, seq))
        {
            return Ok(false);
        ...
        let next = if clear { BTreeMap::new() } else { labels };
        let changed = pane.display_state_labels != next;
    ...
    pub fn release_agent(
        ...
        pane.display_state_labels_authority = None;
        pane.display_state_labels.clear();
